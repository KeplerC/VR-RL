...
class Agent():
    ...
    def build_network(self):
        ...
        s = tf.placeholder(tf.float32, [None, NUM_ANALYZER * NUM_FEATURE, NUM_TIME_STAMP ])
        q_values = model(s)
    ...
    def get_initial_state(self, observation, last_observation):
        processed_observation = observation - last_observation
        return processed_observation

    def get_action(self, state, bound = None):
        ...
        if terminal:
            ...
            if self.t >= INITIAL_REPLAY_SIZE:
                stats = [self.total_reward, self.total_q_max / float(self.duration),
                         self.duration, self.total_loss / (float(self.duration) / float(TRAIN_INTERVAL)), action]
    ...
    def setup_summary(self):
        ...
        action_performed = tf.Variable(0.)
        tf.summary.histogram(ENV_NAME + '/Average Loss/Episode', action_performed)
        
        summary_vars = [episode_total_reward, episode_avg_max_q, episode_duration, episode_avg_loss, action_performed]
...
def main():
    ...
    if TRAIN:  # Train mode
        for _ in range(NUM_EPISODES):
            ...
            observation = observation.reshape((NUM_ANALYZER * NUM_FEATURE , NUM_TIME_STAMP))
            ...
            while not terminal:
                counter += 1
                last_observation = observation
                action = agent.get_action(state, info["bound"])
